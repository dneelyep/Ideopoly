    ...
    public void doActions(Player p, GameBoard board) { // TODO: Rename this to drawCard() or something better.
        switch (this.cardType) {
            case 1: p.setCell(board.boardProperties.get(0)); // "Advance to Go (Collect $200)"
                ...
                break;
            case 2: p.setCash(CASH_TYPES.hundreds, p.getCash(CASH_TYPES.hundreds) + 2);
                break;
                ...
                for (Player player : board.players) {
                    if (player != p) { // Don't collect money from p.
                        if (player.willBankrupt(10)) {
                            ...
                            player.setCash(CASH_TYPES.tens, player.getCash(CASH_TYPES.tens) - 1);
                            player.spreadCash(CASH_TYPES.fiveHundreds);
                        }
                    }
                }

                p.setCash(CASH_TYPES.tens, p.getCash(CASH_TYPES.tens) + numNotBankrupt);
                break;
            case 7:   // "Grand Opera Night – collect $50 from every player for opening night seats"
                ...
                p.setCash(CASH_TYPES.fifties, p.getCash(CASH_TYPES.fifties) + numNotBankrupt);
                break;
            case 8: p.setCash(CASH_TYPES.tens, p.getCash(CASH_TYPES.tens) + 2); // "Income Tax refund – collect $20"
                break;
            case 9: p.setCash(CASH_TYPES.hundreds, p.getCash(CASH_TYPES.hundreds) + 1); // "Life Insurance Matures – collect $100"
                break;
            ...
            case 11:  // "Pay School Fees of $50"
                p.payBank(50, board);
                break;
            case 12: p.setCash(CASH_TYPES.twenties, p.getCash(CASH_TYPES.twenties) + 1); // "Receive $25 Consultancy Fee"
                p.setCash(CASH_TYPES.fives, p.getCash(CASH_TYPES.fives) + 1);
                break;
            case 13:  // "You are assessed for street repairs – $40 per house, $115 per hotel"
                ...
                break;
            case 14: p.setCash(CASH_TYPES.tens, p.getCash(CASH_TYPES.tens) + 1); // "You have won second prize in a beauty contest– collect $10"
                break;
            case 15: p.setCash(CASH_TYPES.hundreds, p.getCash(CASH_TYPES.hundreds) + 1); // "You inherit $100"
                break;
            case 16: p.setCash(CASH_TYPES.fifties, p.getCash(CASH_TYPES.fifties) + 1); // "From sale of stock you get $50"
                break;
            case 17: p.setCash(CASH_TYPES.hundreds, p.getCash(CASH_TYPES.hundreds) + 1); // "Holiday Fund matures - Receive $100"
                break;
